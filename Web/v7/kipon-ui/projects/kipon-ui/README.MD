...
```html
<ko-title-panel>
     <ko-header ko-title>The title of the panel</ko-header>
     <ko-view ko-body>The body of the panel</ko-view>
</ko-title-panel>
```
...
```html
<ko-header>The title goes here</koheader>
```
...
<ko-alert {class="warning"}></ko-alert>
...
```html
<input ko-decimal="number of decimals" [(ko-value)]="yourmodel.decimalfield" {(ko-valueChange)="callMeOnChangeOfTheFieldFunction($event)"} />
```
...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.items = [];
    this.items.push({ id: 'i1', title: 'A', disabled: false });
    this.items.push({ id: 'i2', title: 'B', disabled: false });
